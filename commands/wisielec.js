exports.run = (client, message, args) => {
	...
	if (args[0]=="start"){
		...
		collector.on('collect', m => {
			...
			if (m.content==="wi stop") {collector.stop('koniec'); return;}
			if (literka==haslo)  {message.channel.send("zgadłeś!"); collector.stop('koniec'); return;}
			else if (literka.length!=1) {message.channel.send("nieprawidłowy format literki"); return;}
